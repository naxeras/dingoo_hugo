...
hugo_update_save_name(char *Name)
{
  ...
  SaveName = strrchr(Name, DIRSEP_CHAR);
  ...
  for (index = 0; index < HUGO_MAX_SAVE_STATE; index++) {
    ...
    snprintf(TmpFileName, MAX_PATH, "%s" DIRSEP "save" DIRSEP "sav_%s_%d.stz", HUGO.hugo_home_dir, HUGO.hugo_save_name, index);
    if (! stat(TmpFileName, &aStat)) {
      HUGO.hugo_save_state[index].used = 1;
      HUGO.hugo_save_state[index].date = aStat.st_mtime;
      snprintf(TmpFileName, MAX_PATH, "%s" DIRSEP "save" DIRSEP "sav_%s_%d.png", HUGO.hugo_home_dir, HUGO.hugo_save_name, index);
      if (! stat(TmpFileName, &aStat)) {
  ...
  snprintf(TmpFileName, MAX_PATH, "%s" DIRSEP "txt" DIRSEP "%s.txt", HUGO.hugo_home_dir, HUGO.hugo_save_name);
  if (! stat(TmpFileName, &aStat)) {
...
load_thumb_list()
{
  ...
  snprintf(SaveDirName, MAX_PATH, "%s" DIRSEP "save", HUGO.hugo_home_dir);
...
load_thumb_if_exists(char *Name)
{
  ...
  SaveName = strrchr(FileName, DIRSEP_CHAR);
  if (SaveName != (char *)0) SaveName++;
  ...
  while (scan_list != 0) {
    if (! strcasecmp( SaveName, scan_list->name)) {
      snprintf(ThumbFileName, MAX_PATH, "%s" DIRSEP "save" DIRSEP "%s", HUGO.hugo_home_dir, scan_list->thumb);
...
load_comment_list()
{
  ...
  snprintf(SaveDirName, MAX_PATH, "%s" DIRSEP "txt", HUGO.hugo_home_dir);
...
static char loc_comment_buffer[128];
  ...
  while (scan_list != 0) {
    if (! strcasecmp( SaveName, scan_list->name)) {
      snprintf(TmpFileName, MAX_PATH, "%s" DIRSEP "txt" DIRSEP "%s", HUGO.hugo_home_dir, scan_list->filename);
...
hugo_kbd_load(void)
{
  ...
  snprintf(TmpFileName, MAX_PATH, "%s" DIRSEP "kbd" DIRSEP "%s.kbd", HUGO.hugo_home_dir, HUGO.hugo_save_name );
  if (! stat(TmpFileName, &aStat)) {
...
hugo_joy_load(void)
{
  ...
  snprintf(TmpFileName, MAX_PATH, "%s" DIRSEP "joy" DIRSEP "%s.joy", HUGO.hugo_home_dir, HUGO.hugo_save_name );
  if (! stat(TmpFileName, &aStat)) {
...
hugo_default_settings()
{
  ...
  HUGO.hugo_snd_freq          = SND_STEREO_22K;
...
hugo_save_settings(void)
{
  ...
  snprintf(FileName, MAX_PATH, "%s" DIRSEP "set" DIRSEP "%s.set", HUGO.hugo_home_dir, HUGO.hugo_save_name);
...
hugo_save_cheat(void)
{
  ...
  snprintf(FileName, MAX_PATH, "%s" DIRSEP "cht" DIRSEP "%s.cht", HUGO.hugo_home_dir, HUGO.hugo_save_name);
...
hugo_snapshot_save_slot(int save_id)
{
  ...
  if (save_id < HUGO_MAX_SAVE_STATE) {
    snprintf(FileName, MAX_PATH, "%s" DIRSEP "save" DIRSEP "sav_%s_%d.stz", HUGO.hugo_home_dir, HUGO.hugo_save_name, save_id);
    ...
    if (! error) {
      if (! stat(FileName, &aStat)) {
        ...
        snprintf(FileName, MAX_PATH, "%s" DIRSEP "save" DIRSEP "sav_%s_%d.png", HUGO.hugo_home_dir, HUGO.hugo_save_name, save_id);
...
hugo_snapshot_load_slot(int load_id)
{
  ...
  if (load_id < HUGO_MAX_SAVE_STATE) {
    snprintf(FileName, MAX_PATH, "%s" DIRSEP "save" DIRSEP "sav_%s_%d.stz", HUGO.hugo_home_dir, HUGO.hugo_save_name, load_id);
...
psp_global_initialize()
{
  ...
  getcwd(HUGO.hugo_home_dir, sizeof(HUGO.hugo_home_dir));
  hugo_default_settings();
  ...
  if (psp_sdl_init() == 0)
  {
        debug_printf("psp_sdl_init FAILED\n");
        return 0; /* call psp exit? */
  }
