...
getDir(const char *path) 
{
  ...
  while(nfiles<GP2X_FMGR_MAX_ENTRY){
    ...
    if(files[nfiles].d_type == DT_DIR) {
      strcat(files[nfiles].d_name, DIRSEP);
      sortfiles[nfiles] = files + nfiles;
      nfiles++;
      continue;
    }
...
static  int sel=0;
  ...
  for(;;) 
  {
    ...
    #define TRUNC_FILENAME_LEN sizeof(buffer)-1 /* doesn't work */
    #define TRUNC_FILENAME_LEN 28 /* original */
    #define TRUNC_FILENAME_LEN 50
      strncpy(buffer, sortfiles[top+i]->d_name, TRUNC_FILENAME_LEN);
      string_fill_with_space(buffer, TRUNC_FILENAME_LEN);
...
psp_fmgr_menu(int format)
{
  ...
  if (first) {
    ...
    strcat(user_filedir_kbd, DIRSEP "kbd" DIRSEP);
    strcat(user_filedir_joy, DIRSEP "joy" DIRSEP);
    strcat(user_filedir_set, DIRSEP "set" DIRSEP);
    strcat(user_filedir_cht, DIRSEP "cht" DIRSEP);
    strcat(user_filedir_cd , DIRSEP "cd-roms" DIRSEP);
    strcat(user_filedir_rom, DIRSEP "roms" DIRSEP);
/*
** stub code for alternate rom location
{
    strcat(user_filedir_cd , DIRSEP);
    strcat(user_filedir_rom, DIRSEP);
  }
*/
  }
