...
{
  background_surface = IMG_Load("." DIRSEP "menu.png");
  thumb_surface = IMG_Load("." DIRSEP "thumb.png");
}
...
{
  if (! splash_surface) {
    splash_surface = IMG_Load("." DIRSEP "splash.png");
  }
...
{
  if (! help_surface) {
    help_surface = IMG_Load("." DIRSEP "help.png");
  }
...
psp_sdl_init(void)
{
  if (SDL_Init(SDL_INIT_VIDEO|SDL_INIT_AUDIO
      ...
      debug_printf("SDL not open: %s\n", SDL_GetError());
      fprintf(stderr, "SDL not open: %s\n",SDL_GetError()); /* doesn't work under dingoo native */
     return 0;
  }
  ...
  if (SDL_InitSubSystem(SDL_INIT_VIDEO) < 0) {
    debug_printf("SDL SDL_InitSubSystem error: %s\n", SDL_GetError());
    fprintf(stderr, "SDL SDL_InitSubSystem : %s\n",SDL_GetError()); /* doesn't work under dingoo native */
    return 0;
  }
  ...
  if ( !back_surface) {
    debug_printf("SDL back_surface video error: %s\n", SDL_GetError());
    fprintf(stderr, "SDL back_surface video: %s\n",SDL_GetError()); /* doesn't work under dingoo native */
    return 0;
  }
